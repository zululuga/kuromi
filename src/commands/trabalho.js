...
async function runWork(source, reply, channel) {
  ...
  if (!account.profession || !professions[account.profession]) {
    await reply('❌ Escolha uma profissão primeiro com `/profissao`.');
    return;
  }

  const status = getWorkStatus(user.id);
  if (!status.available) {
    await reply(`⏳ Você já trabalhou. Tente novamente em **${formatRemaining(status.remainingMs)}**.`);
    return;
  }
  ...
  await reply(
    `💼 Trabalho de **${professions[account.profession].label}** iniciado!\n` +
      `Envie estas 5 palavras em uma única mensagem, em até 1 minuto:\n**${selectedWords.join(' • ')}**`
  );
  ...
  if (!answerMessage) {
    await reply('⌛ Tempo esgotado. Você não recebeu salário e poderá trabalhar novamente em 3 horas.');
    return;
  }

  if (!isCorrectAnswer(answerMessage.content, selectedWords)) {
    await reply('❌ Uma ou mais palavras estão erradas. Você não recebeu salário e poderá tentar novamente em 3 horas.');
    return;
  }

  const salary = randomSalary();
  const result = finishWork(user.id, true, salary);
  await reply(`✅ Trabalho concluído! Você recebeu **${formatCoins(result.amount)}**. Saldo: **${formatCoins(result.balance)}**.`);
}
