...
async function executeDivorce(source, reply) {
  ...
  if (!spouseId) {
    await reply('❌ Você não está casado(a).');
    return;
  }

  if (getBalance(user.id) < DIVORCE_COST) {
    await reply(`❌ O divórcio custa **${formatCoins(DIVORCE_COST)}**. Seu saldo é **${formatCoins(getBalance(user.id))}**.`);
    return;
  }

  const payment = spendCoins(user.id, DIVORCE_COST);
  if (!payment.spent) {
    await reply(`❌ Você precisa de **${formatCoins(DIVORCE_COST)}** para se divorciar.`);
    return;
  }

  endMarriage(user.id);
  await reply(`💔 Divórcio concluído. Foram cobradas **${formatCoins(DIVORCE_COST)}**.`);
}
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName(DIVORCE)
    .setDescription('Encerra seu casamento por 500 Moedinhas.'),
